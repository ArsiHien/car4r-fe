...
const RevenueByCarCateChart: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRevenueByCateChart = async () => {
            try {
                const response = await axios.get("http://localhost:8080/api/management/revenue/by-category-type");
                const data = await response.data;
                const labels = data.map((element: { categoryType: string }) => element.categoryType);
